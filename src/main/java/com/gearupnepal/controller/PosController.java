...
import com.gearupnepal.web.SimpleAuthenticationSuccessHandler;
 ...
 */
@Controller
@RequestMapping(value = {"/admin/pos", "/user/pos"})
...
//    @Autowired
//    SimpleAuthenticationSuccessHandler authority;
    ...
    public String index(Model model) {
        ...
        model.addAttribute("authority", getRole());
        tempChilds.clear();
...
if (wrapper.getUsers().size() == 0) {
            ...
            tempChilds.add(new TempChild(id, childSubcategoryService.findById(id).getName(),
                    ...
                    System.out.println(" if adding");

        } else {
            System.out.println("size not 0");

            for (TempChild tcw : wrapper.getUsers()) {
                if (!tcw.getName().equals(childSubcategoryService.findById(id).getName())) {
                    tempChilds.add(new TempChild(id, childSubcategoryService.findById(id).getName(),
                            childSubcategoryService.findById(id).getQuantity(), childSubcategoryService.findById(id).getPrice()));
                        System.out.println(" else adding");
