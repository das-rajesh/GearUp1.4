            ...
            <div class="col-md-4">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <%
                                ArrayList<TempChild> tempChilds = (ArrayList<TempChild>) session.getAttribute("tempChilds");
                                for (TempChild tempChild1 : tempChilds) {
                            %>
                            <c:set var="evenCount" value="${evenCount+1}" />
                            <% tempChild = new TempChild();%>
                    <form method="post" action="/POS/change/${evenCount}">

                        <th scope="row"><%= id1%><%tempChild.setId(id1);%></th>
                        <td><%=tempChild1.getName()%><%tempChild.setName(tempChild1.getName());%></td>  
                        <td><div class="quantity">
                                <input type="number" name="quantity" min="1" max="<%=tempChild1.getQuantity()%>;" step="1" value="<%=tempChild1.getQuantity()%>">
                                <% tempChild.setQuantity(tempChild1.getQuantity());%>
                            </div>
                        </td>                    
                        <td><%=tempChild1.getPrice()%><%tempChild.setId(2);%></td>
                        <input type="hidden" name="name" value="<%=tempChild1.getName()%>" />
                        <input type="hidden" name="price" value="<%=tempChild1.getPrice()%>" />
